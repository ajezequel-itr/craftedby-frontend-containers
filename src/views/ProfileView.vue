<template>
  <div class="wrapper mx-auto w-full lg:w-1/2">
    <div class="mt-6">
    ...
    <div class="mt-8">
      <p class="text-lg mb-4">Avis laissés sur produits achetés</p>
      <div v-if="currentUser.user && currentUser.user.reviews && currentUser.user.reviews.length > 0" class="space-y-4">
        <router-link v-for="(review, index) in currentUser.user.reviews" :key="index" :to="`/products/${review.product_id}`" class="block">
          <div class="p-4 bg-gray-100 rounded-lg shadow hover:bg-gray-200">
            <p class="font-semibold text-lg">Evaluation: {{ review.rating }}/5</p>
            <p class="text-gray-600">Commentaire: {{ review.comment }}</p>
            <div class="text-right text-sm text-gray-500">Avis laissé le: {{ new Date(review.created_at).toLocaleDateString() }}</div>
          </div>
        </router-link>
      </div>
